...
class FileDownloadMixIn:
    ...
    def _get_chunk_url(self, uuid: UUID, bucket: str, region: str, chunk: int) -> str:
        base_url = self._context.get_egest_url().rstrip('/')
        return f'{base_url}/{region}/{bucket}/{uuid}/{chunk}'


class FileDownloadAPI(APIBase, FileDownloadMixIn):
    """API for downloading files from the storage"""

    def chunk(self, uuid: UUID, bucket: str, region: str, chunk: int) -> bytes:
        """Download a file encrypted chunk"""

        chunk_url = self._get_chunk_url(uuid, bucket, region, chunk)
        headers = self._ensure_api_key(use_api_key=True, url=chunk_url)

        with self._request_error_handler:
            with self._http_client.stream('GET', chunk_url, headers=headers) as resp:
...
class AsyncFileDownloadAPI(AsyncAPIBase, FileDownloadMixIn):
    ...
    async def chunk(self, uuid: UUID, bucket: str, region: str, chunk: int) -> bytes:
        """Download a file encrypted chunk"""
        ...
        with self._request_error_handler:
            async with self._http_client.stream('GET', chunk_url, headers=headers) as resp:
                resp.raise_for_status()
                buffer = bytearray()
